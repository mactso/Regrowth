...
public class MushroomActionHelpers2 {
	...
	static boolean isValidMushroomPosition(ActionContext rgCtx, double growthFilterValue) {
		...
		if (!MushroomActionHelpers2.isGoodMushroomTemperature(rgCtx))
			return false;
	
		// Check for nearby mushroom stems in a 7x7 horizontal box, 1 block tall
		if (MushroomActionHelpers2.isNearbyMushroomStem(serverLevel, pos)) {
			return false;
		}
