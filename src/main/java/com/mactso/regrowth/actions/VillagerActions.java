...
public class VillagerActions {
	...
	static void vClericalHealing(ActionContext rgCtx) {
		...
		// Only clerics heal
		if (!ve.getVillagerData().profession().is(VillagerProfession.CLERIC))
			return;
		...
		if (!(result.isEmpty())) {
			...
			while (i.hasNext()) { // in 16.1, finds the point of interest.
				...
				if ((vHomePos.getX() == P.getPos().getX()) && (vHomePos.getY() == P.getPos().getY())
					...
					MyUtilities.debugMsg(1, P.getPos(), "extra Point of Interest Found.");
...
//falls through if not a cuttable plant
	...
	static void vImproveVillageWalls(ActionContext rgCtx) {
		...
		if (!ActionTests.isWallBuildingOff()) // configured wall block is blank or not a valid block
			return;
		...
		// tested above to see there if there was a valid configured wall control block
		...
		// has the player removed the wall building control block for this village?
		Block controlWallBlock = serverLevel.getBlockState(controlWallPos).getBlock();
		if (controlWallBlock != configuredWallControlBlock) {
			return; // or return false if inside a boolean method
		}
	...
	static boolean vImproveVillageWall(ActionContext rgCtx, BlockPos villageMeetingPointPos) {
		...
		if (ve.getVillagerData().profession().is(VillagerProfession.MASON)) {
	...
	static boolean vToolMasterHealing(ActionContext rgCtx) {
		...
		int villagerLevel = ve.getVillagerData().level() + 1;
