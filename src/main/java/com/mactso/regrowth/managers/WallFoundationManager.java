...
public class WallFoundationManager {
    ...
    private static <T> Registry<T> getRegistrySafe(RegistryAccess access, net.minecraft.resources.ResourceKey<Registry<T>> key) {
        return access.lookup(key)
                .orElseThrow(() -> new IllegalStateException("Registry not found: " + key.location()));
    }
